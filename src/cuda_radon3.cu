...
namespace cuda {
   ...
   )
   {
      ...
      si.set_image(
         j_1 - j_0, k_1 - k_0,
         image.tlf_x + k_0 * ( ( image.brb_y - image.tlf_y ) / image.o ),
         image.tlf_y + j_0 * ( ( image.brb_z - image.tlf_z ) / image.n ),
         image.tlf_x + k_1 * ( ( image.brb_y - image.tlf_y ) / image.o ),
         image.tlf_y + j_1 * ( ( image.brb_z - image.tlf_z ) / image.n )
      );
